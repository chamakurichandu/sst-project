...
export default function ReleasedMaterials(props) {
  ...
  async function getReleasedTransactions(numberOfRows, search = "") {
    try {
      setShowBackDrop(true);
      console.log("page: ", page);
      let url = config["baseurl"] + "/api/releasetransaction/list?count=" + numberOfRows + "&warehouse=" + props.warehouse._id + "&offset=" + offset + "&search=" + search;
  ...
  const handleChangeRowsPerPage = (event) => {
    ...
    setPage(0);
    offset = 0;
    getReleasedTransactions(newRowsPerPage);
  };
  ...
  const onSearchChange = (event) => {
    ...
    getReleasedTransactions(rowsPerPage, event.target.value);
  };
