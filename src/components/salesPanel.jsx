...
function EnhancedTableHead(props) {
  ...
  const headCells = [
    ...
    { id: 'visit_stall', numeric: false, disablePadding: false, label: 'Actions' },
  ];
...
export default function SalesPanel(props) {
  ...
  useEffect(() => {
    ...
    props.sendGetSalesVideoCallQueue(profileData["user_id"]);
  ...
  console.log("props.salesVideoCallMembers: ", props.salesVideoCallMembers);
  return (
    <div className={clsx(classes.root)}>
      {props.refreshUI &&
        <div className={classes.paper}>
          <EnhancedTableToolbar title={lstrings.SalesPanel} />
          <Paper className={classes.grid}>
            <Grid container spacing={2}>
              <Grid item className={classes.totalAttendes}>
                <img src={LaptopImage} width='25' alt="" />
                <h1 className={classes.h1}>{props.salesVideoCallMembers.length}</h1>
                <span>{lstrings.DelegatesInQueue}</span>
              </Grid>
            </Grid>
          </Paper>
          <Paper className={classes.grid}>
                        ...
                        >
                          ...
                          <TableCell align={dir === 'rtl' ? 'right' : 'left'}>
                            <div className={classes.flex}>
                              ...
                              <span>
                                {props.salesVideoCallMembersNames ? props.salesVideoCallMembersNames[index] : row}
                              </span>
                            </div>
                          </TableCell>
                          <TableCell align={dir === 'rtl' ? 'right' : 'left'}>
                            <div style={{ paddingTop: 10 }}><Button onClick={() => props.sendAcceptForSalesCall(row)} style={{ background: "#314293", color: "#FFFFFF" }} variant="contained" className={classes.button}>{lstrings.Accept}</Button></div>
                            <div style={{ paddingTop: 10 }}><Button onClick={() => props.sendRejectForSalesCall(row)} style={{ background: "#d10d4e", color: "#FFFFFF" }} variant="contained" className={classes.button}>{lstrings.Reject}</Button></div>
                          </TableCell>
                        </TableRow>
                      );
                    })}
