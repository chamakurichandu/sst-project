...
export default function SupplyVendors(props) {
  ...
  async function getList(numberOfRows, search = "") {
    try {
      ...
      let url = config["baseurl"] + "/api/supplyvendor/list?count=" + numberOfRows + "&offset=" + offset + "&search=" + search;
  ...
  const handleEdit = (data) => {
    ...
    props.history.push("/editsupplyvendor");
  };

  const handleAdd = () => {
    props.history.push("/addsupplyvendor");
  };
    ...
    <div className={clsx(classes.root)}>
      {props.refreshUI &&
        ...
        <div className={classes.paper}>
          <EnhancedTableToolbar title={lstrings.SupplyVendor} />
          <Paper className={classes.grid}>
            <Grid container spacing={2}>
              <Grid item className={classes.totalAttendes}>
                <img src={ShipImage} width='25' alt="" />
                <h1 className={classes.h1}>{totalCount}</h1>
                <span>{lstrings.SupplyVendor}</span>
              </Grid>
              <Grid item className={classes.addButton}>
                <Button onClick={() => handleAdd()} style={{ background: "#314293", color: "#FFFFFF" }} variant="contained" className={classes.button}>{lstrings.AddSupplyVendor}</Button>
              </Grid>
            </Grid>
          </Paper>
          <Paper className={classes.grid}>
