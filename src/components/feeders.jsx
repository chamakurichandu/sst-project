...
export default function Feeders(props) {
  ...
  async function getList(numberOfRows, search = "") {
    ...
    try {
      ...
      let url = config["baseurl"] + "/api/feeder/list?section=" + sections[index]._id + "&count=" + 10000 + "&offset=" + 0 + "&search=" + search;
      axios.defaults.headers.common['authToken'] = window.localStorage.getItem("authToken");
      const { data } = await axios.get(url);
      // console.log(data);
  ...
  const handleDelete = async () => {
    try {
      setShowBackdrop(true);
      let url = config["baseurl"] + "/api/feeder/delete";
  ...
  return (
    <div className={clsx(classes.root)}>
      ...
      {showNewItem && <AddFeeder closeAction={closeNewDialogAction} onNewSaved={onNewSaved} section={sections[currentSection]} />}
      {showEditItem && <EditFeeder closeAction={closeEditDialogAction} deleteAction={deleteAction} onNewSaved={onNewSaved} item={selectedItem} />}
      {showDeleteConfirmationDialog && <ConfirmationDialog noConfirmationDialogAction={noConfirmationDialogAction} yesConfirmationDialogAction={yesConfirmationDialogAction} message={"Are you sure you want to delete feeder: " + selectedItem.name} title={"Deleting Feeder"} />}
