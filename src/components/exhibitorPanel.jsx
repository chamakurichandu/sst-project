...
function EnhancedTableHead(props) {
  ...
  const headCells = [
    ...
    { id: 'stall_id', numeric: false, disablePadding: false, label: 'Stall Id' },
    { id: 'stall_type', numeric: false, disablePadding: false, label: 'Stall Type' },
...
export default function ExhibitorPanel(props) {
  ...
  async function getStallsInfo() {
    console.log("getStallsInfo");
    try {
      let sessionData = await Auth.currentSession();
      try {
        let url = config["baseurl"] + "/org/" + config["org"] + "/event/" + config["event"] + "/profile/stall";
        ...
        let newRows = [];
        let newlogos = [];
        for (let i = 0; i < data.response.stalls.length; ++i) {
          newRows.push(createData((i + 1), data.response.stalls[i]));
          newlogos.push(data.response.stalls[i].logo_url);
        }
        setLogos(newlogos);
        setRows(newRows);
      }
      catch (e) {
        console.log("Error in getting Stalls list");
  ...
  useEffect(() => {
    console.log("Sales Panel : useEffect() ");
    getStallsInfo();
  ...
  const handleEdit = (row) => {
    props.setCurrentExhibitorStallInfo(row.data);
    props.history.push("/exhibitor-edit-stall");
  };
    ...
    <div className={clsx(classes.root)}>
      {props.refreshUI &&
        <div className={classes.paper}>
          <EnhancedTableToolbar title={lstrings.ExhibitorPanel} />
          {/* <Paper className={classes.grid}>
            <Grid container spacing={2}>
              <Grid item className={classes.totalAttendes}>
                <img src={PanelImage} width='25' alt="" />
                <h1 className={classes.h1}>{0}</h1>
                <span>{lstrings.Visited}</span>
              </Grid>
            </Grid>
          </Paper> */}
          <Paper className={classes.grid}>
                  ...
                  {rows.map((row, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell align={dir === 'rtl' ? 'right' : 'left'}>
                          <div style={{ paddingTop: 10 }}><Button onClick={() => handleEdit(row)} style={{ background: "#314293", color: "#FFFFFF" }} variant="contained" className={classes.button}>{lstrings.Edit}</Button></div>
                        </TableCell>
                      </TableRow>
                    );
                  })}
