...
export default function WorkAcceptance(props) {
  ...
  const handleComplete = async () => {
    try {
      ...
      for (let i = 0; i < items.length; ++i) {
        if(parseInt(items[i].testing_qty) !== parseInt(items[i].acceptance_qty)) {
          throw { message: "Testing qty and Acceptance qty should be same" };
        }
      }
        ...
        <Paper className={classes.grid}>
          <Grid container spacing={2}>
            <Grid item className={classes.totalAttendes}>
              ...
              <span>{"Materials in Acceptance"}</span>
            </Grid>
            <Grid item className={classes.addButton}>
              {!editMode && <Button onClick={() => handleComplete()} style={{ background: "#314293", color: "#FFFFFF", marginLeft: 5 }} variant="contained" className={classes.button}>{"Complete Acceptance"}</Button>}
              <Button onClick={() => handleSave()} style={{ marginLeft: 10, background: "#314293", color: "#FFFFFF" }} variant="contained" className={classes.button}>{editMode ? "Save Acceptance" : "Edit Acceptance"}</Button>
