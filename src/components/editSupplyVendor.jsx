...
export default function EditSupplyVendor(props) {
  ...
  const [code, set_code] = React.useState(props.selectedSupplyVendor.code);

  const [name, set_name] = React.useState(props.selectedSupplyVendor.name);
  const [name_error, set_name_error] = React.useState(null);

  const [website, set_website] = React.useState(props.selectedSupplyVendor.website);
  const [website_error, set_website_error] = React.useState(null);

  const [address, set_address] = React.useState(props.selectedSupplyVendor.address);
  const [address_error, set_address_error] = React.useState(null);

  const [billingAddress, set_billingAddress] = React.useState(props.selectedSupplyVendor.billingAddress);
  const [billingAddress_error, set_billingAddress_error] = React.useState(null);

  const [officePhone, set_officePhone] = React.useState(props.selectedSupplyVendor.officePhone);
  const [officePhone_error, set_officePhone_error] = React.useState(null);

  const [gst, set_gst] = React.useState(props.selectedSupplyVendor.gst);
  const [gst_error, set_gst_error] = React.useState(null);

  const [contactName, set_contactName] = React.useState(props.selectedSupplyVendor.contactName);
  const [contactName_error, set_contactName_error] = React.useState(null);

  const [contactEmail, set_contactEmail] = React.useState(props.selectedSupplyVendor.contactEmail);
  const [contactEmail_error, set_contactEmail_error] = React.useState(null);

  const [contactPhone, set_contactPhone] = React.useState(props.selectedSupplyVendor.contactPhone);
  const [contactPhone_error, set_contactPhone_error] = React.useState(null);

  const [city, set_city] = React.useState(props.selectedSupplyVendor.city);
  const [city_error, set_city_error] = React.useState(null);

  const [district, set_district] = React.useState(props.selectedSupplyVendor.district);
  const [district_error, set_district_error] = React.useState(null);

  const [state, set_state] = React.useState(props.selectedSupplyVendor.state);
  const [state_error, set_state_error] = React.useState(null);

  const [country, set_country] = React.useState(props.selectedSupplyVendor.country);
  const [country_error, set_country_error] = React.useState(null);

  const [latlong, set_latlong] = React.useState(props.selectedSupplyVendor.latlong);
  const [latlong_error, set_latlong_error] = React.useState(null);

  const [files, set_files] = React.useState(props.selectedSupplyVendor.docs);
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      try {
        let url = config["baseurl"] + "/api/supplyvendor/update";
        ...
        console.log("props.selectedSupplyVendor._id: ", props.selectedSupplyVendor._id);
        let updateObj = { _id: props.selectedSupplyVendor._id, updateParams: postObj };
  ...
  const handleDelete = async () => {
    ...
    try {
      let url = config["baseurl"] + "/api/supplyvendor/delete";

      let postObj = {};
      postObj["_id"] = props.selectedSupplyVendor["_id"];
    ...
    <div className={clsx(classes.root)}>
      {props.refreshUI &&
        ...
        <div className={classes.paper}>
          ...
          <EnhancedTableToolbar title={lstrings.EditSupplyVendor} />

          <Breadcrumbs aria-label="breadcrumb">
            <Link color="inherit" onClick={handleBreadCrumUsersClick}>
              {lstrings.SupplyVendors}
            </Link>
            <Typography color="textPrimary">{lstrings.EditSupplyVendor}</Typography>
          </Breadcrumbs>
