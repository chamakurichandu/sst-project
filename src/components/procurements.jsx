...
export default function Procurements(props) {
  ...
  async function getList(numberOfRows, search = "") {
    try {
      ...
      let url = config["baseurl"] + "/api/procurement/list?count=" + numberOfRows + "&offset=" + offset + "&search=" + search;
  ...
  const handleAdd = () => {
    props.history.push("/addprocurement");
  };
    ...
    <div className={clsx(classes.root)}>
      {props.refreshUI &&
        ...
        <div className={classes.paper}>
          <EnhancedTableToolbar title={"Procurements"} />
          <Paper className={classes.grid}>
            <Grid container spacing={2}>
              ...
              <Grid item className={classes.addButton}>
                <Button onClick={() => handleAdd()} style={{ background: "#314293", color: "#FFFFFF" }} variant="contained" className={classes.button}>{"Add Procurement"}</Button>
              </Grid>
            </Grid>
          </Paper>
          <Paper className={classes.grid}>
