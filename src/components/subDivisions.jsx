...
export default function SubDivision(props) {
  ...
  async function getList(numberOfRows, search = "") {
    ...
    try {
      ...
      const index = currentDivision === -1 ? 0 : currentDivision;
      console.log("divisions[currentDivision]: ", divisions[index]);

      let url = config["baseurl"] + "/api/subdivision/list?division=" + divisions[index]._id + "&count=" + 10000 + "&offset=" + 0 + "&search=" + search;
      axios.defaults.headers.common['authToken'] = window.localStorage.getItem("authToken");
      const { data } = await axios.get(url);
      console.log(data);
  ...
  const handleDelete = async () => {
    try {
      setShowBackdrop(true);
      let url = config["baseurl"] + "/api/subdivision/delete";
  ...
  return (
    <div className={clsx(classes.root)}>
        ...
        <Paper className={classes.grid}>
          <Grid container spacing={2}>
            ...
            <Grid item className={classes.addButton}>
              <Button onClick={() => handleAdd()} style={{ background: "#314293", color: "#FFFFFF" }} variant="contained" className={classes.button}>{"Add SubDivision"}</Button>
            </Grid>
          </Grid>
        </Paper>
        <Paper className={classes.grid}>
      ...
      {showNewItem && <AddSubDivision closeAction={closeNewDialogAction} onNewSaved={onNewSaved} division={divisions[currentDivision]} />}
      {showEditItem && <EditSubDivision closeAction={closeEditDialogAction} deleteAction={deleteAction} onNewSaved={onNewSaved} item={selectedItem} />}
